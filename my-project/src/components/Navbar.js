                ...
                <div className="dropdown hidden sm:flex sm:flex-col">
                    ...
                    <div className="dropdown-content hidden sm:w-36 sm:h-full pb-3 ">
                        <div className="flex flex-col-reverse sm:w-36 bg-white/75">
                            {projets.map((projet) => ( 
                            <Link key={projet.id} href={`#section_${projet.id}`}><p className="text-black text-sm p-2 cursor-pointer hover:text-[#F2511B] hover:font-medium">{projet.title} <span className="text-xs font-thin">- {projet.techno}</span></p></Link>
                            ))}
                        </div>
                    </div>
                </div>
